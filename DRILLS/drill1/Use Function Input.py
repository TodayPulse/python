# Write a function called `solution` that receives a student's 
# name and learning track, then returns a short profile sentence.

# The sentence must follow this exact format:

# <name> is starting the <track> track.

# Rules:
# - Use both function arguments.
# - Return the final string.
# - Do not print.
# - The output must match exactly.

def solution(name, track):
    return f"{name} is starting the {track} track."

print(solution("Olowu Emmanuel","Python"))